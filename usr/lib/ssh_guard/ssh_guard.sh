...
parse_auth_log() {
	...
	logread 2>/dev/null | tail -n "$new_lines" | grep "sshd" | while read -r line; do
		if echo "$line" | grep -qi "failed password\|invalid user\|authentication failure"; then
